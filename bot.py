...
# Create default SSL context for certificate verification
...
DEFAULT_HEALTH_CHECK_TIMEOUT = 8
DEFAULT_WEB_SEARCH_TIMEOUT = 7
DEFAULT_AUTOMATION_LOOP_INTERVAL = 5
DEFAULT_REPLY_DELAY_MIN = 0.1
DEFAULT_REPLY_DELAY_MAX = 0.4

# TTS Cache defaults
DEFAULT_TTS_CACHE_MAX_ITEMS = 10
DEFAULT_TTS_CACHE_MAX_BYTES = 5_000_000
DEFAULT_TTS_CACHE_TTL_SECONDS = 3600

# Rubika defaults
DEFAULT_RUBIKA_CONFIRM_TTL = 900  # 15 min
DEFAULT_RUBIKA_CHAT_REF_TTL = 7 * SECONDS_PER_DAY
DEFAULT_RUBIKA_MESSAGE_REF_TTL = 24 * SECONDS_PER_HOUR
DEFAULT_MAX_RUBIKA_REFS = 200
DEFAULT_MAX_RUBIKA_MESSAGE_REFS = 500
DEFAULT_MAX_RUBIKA_PENDING_ACTIONS = 50
...
# Automation defaults
DEFAULT_MAX_REMINDERS = 100
DEFAULT_MAX_MONITORS = 20
DEFAULT_MAX_OUTBOX_EVENTS = 300
DEFAULT_MAX_SERVER_FILES = 50
DEFAULT_MAX_TASKS = 200
DEFAULT_MAX_SERVER_FILE_BYTES = 100_000

# Voice defaults
DEFAULT_VOICE_MAX_BYTES = 10_000_000
DEFAULT_VOICE_MAX_SECONDS = 60
DEFAULT_VOICE_TTS_MAX_CHARS = 1500
DEFAULT_VOICE_MAX_OUTPUT_BYTES = 3_000_000
DEFAULT_VOICE_PROCESSING_CONCURRENCY = 2

# Rate limiting (requests per minute per IP) – قابل تنظیم با ENV
DEFAULT_API_RATE_LIMIT = int(_float_env("RL_API", 60, 5, 6000))
DEFAULT_DASHBOARD_RATE_LIMIT = int(_float_env("RL_DASHBOARD", 30, 5, 6000))
DEFAULT_DOWNLOAD_RATE_LIMIT = int(_float_env("RL_DOWNLOAD", 30, 3, 600))
DEFAULT_CHAT_RATE_LIMIT = int(_float_env("RL_CHAT", 20, 1, 600))
DEFAULT_VOICE_RATE_LIMIT = int(_float_env("RL_VOICE", 5, 1, 120))
DEFAULT_SEND_RATE_LIMIT = int(_float_env("RL_SEND", 10, 1, 600))
DEFAULT_KB_RATE_LIMIT = int(_float_env("RL_KB", 30, 1, 600))
DEFAULT_KB_WRITE_RATE_LIMIT = int(_float_env("RL_KB_WRITE", 10, 1, 600))
DEFAULT_PENDING_RATE_LIMIT = int(_float_env("RL_PENDING", 30, 1, 600))
DEFAULT_ANSWER_RATE_LIMIT = int(_float_env("RL_ANSWER", 20, 1, 600))
DEFAULT_LOGS_RATE_LIMIT = int(_float_env("RL_LOGS", 30, 1, 600))
DEFAULT_BRIEFING_RATE_LIMIT = int(_float_env("RL_BRIEFING", 10, 1, 600))

# GEMINI Search cooldown
DEFAULT_GEMINI_SEARCH_COOLDOWN = 600

# File size limits
MAX_JSON_FILE_SIZE = 2_000_000  # 2MB for agent data files

# Logging
MAX_CHAT_LOGS = 2000
MAX_BOT_SENT_IDS = int(_float_env("MAX_BOT_SENT_IDS", 5000, 100, 200_000))

# ── باگ #14: مقادیر واقعی مصرفی از همین DEFAULT_*ها ساخته می‌شوند ──
# قبلاً بلوک DEFAULT_* کاملاً مرده بود و در جای دیگری از فایل همان اعداد
# به‌صورت hardcode دوباره نوشته شده بودند؛ تغییر یکی، دیگری را به‌روز نمی‌کرد.
...
MAX_AGENT_HISTORY_ITEMS = int(
    _float_env("MAX_AGENT_HISTORY_ITEMS", DEFAULT_MAX_AGENT_HISTORY, 4, 400)
)
MAX_REMINDERS = int(_float_env("MAX_REMINDERS", DEFAULT_MAX_REMINDERS, 5, 5000))
MAX_MONITORS = int(_float_env("MAX_MONITORS", DEFAULT_MAX_MONITORS, 1, 500))
MAX_OUTBOX_EVENTS = int(
    _float_env("MAX_OUTBOX_EVENTS", DEFAULT_MAX_OUTBOX_EVENTS, 10, 10000)
)
MAX_SERVER_FILES = int(_float_env("MAX_SERVER_FILES", DEFAULT_MAX_SERVER_FILES, 1, 1000))
MAX_TASKS = int(_float_env("MAX_TASKS", DEFAULT_MAX_TASKS, 5, 5000))
MAX_SERVER_FILE_BYTES = int(
    _float_env("MAX_SERVER_FILE_BYTES", DEFAULT_MAX_SERVER_FILE_BYTES, 1000, 5_000_000)
)
AUTOMATION_LOOP_SECONDS = _float_env(
    "AUTOMATION_LOOP_SECONDS", DEFAULT_AUTOMATION_LOOP_INTERVAL, 1, 120
)
HEALTH_CHECK_TIMEOUT_SECONDS = _float_env(
    "HEALTH_CHECK_TIMEOUT_SECONDS", DEFAULT_HEALTH_CHECK_TIMEOUT, 2, 60
)
WEB_SEARCH_TIMEOUT_SECONDS = _float_env(
    "WEB_SEARCH_TIMEOUT_SECONDS", DEFAULT_WEB_SEARCH_TIMEOUT, 3, 20
)
REPLY_DELAY_MIN = _float_env("REPLY_DELAY_MIN", DEFAULT_REPLY_DELAY_MIN, 0, 5)
REPLY_DELAY_MAX = _float_env(
    "REPLY_DELAY_MAX", max(DEFAULT_REPLY_DELAY_MAX, REPLY_DELAY_MIN), REPLY_DELAY_MIN, 8
)
GEMINI_SEARCH_COOLDOWN_SECONDS = _float_env(
    "GEMINI_SEARCH_COOLDOWN_SECONDS", DEFAULT_GEMINI_SEARCH_COOLDOWN, 30, 3600
)
...
# روی هاست‌هایی مثل Render/Railway که فایل‌سیستم موقتی است، با هر ری‌استارت همه چیز
# پاک می‌شد. حالا یک DATA_DIR واحد داریم که می‌توان آن را روی دیسک پایدار mount کرد.
# پیش‌فرض "." است تا رفتار قبلی برای کسی که چیزی تنظیم نکرده تغییری نکند.
DATA_DIR = os.environ.get("DATA_DIR", ".").strip() or "."


def _data_path(filename: str) -> str:
    """مسیر یک فایل داده را داخل DATA_DIR برمی‌گرداند (مسیر مطلق را دست‌نخورده رد می‌کند)."""
    name = str(filename or "").strip()
    if not name:
        raise ValueError("filename خالی است")
    if os.path.isabs(name) or DATA_DIR in (".", ""):
        return name
    os.makedirs(DATA_DIR, exist_ok=True)
    return os.path.join(DATA_DIR, name)


AGENT_MEMORY_FILE = _data_path(
    os.environ.get("AGENT_MEMORY_FILE", "agent_memory.json").strip()
)
AGENT_AUDIT_FILE = _data_path(
    os.environ.get("AGENT_AUDIT_FILE", "agent_audit.json").strip()
)
AUTOMATION_FILE = _data_path(
    os.environ.get("AUTOMATION_FILE", "server_automation.json").strip()
)
SERVER_FILES_DIR = _data_path(
    os.environ.get("SERVER_FILES_DIR", "server_files").strip()
)
...
VOICE_TTS_MAX_CHARS = int(_float_env("VOICE_TTS_MAX_CHARS", DEFAULT_VOICE_TTS_MAX_CHARS, 100, 3000))
VOICE_MAX_OUTPUT_BYTES = int(
    _float_env("VOICE_MAX_OUTPUT_BYTES", DEFAULT_VOICE_MAX_OUTPUT_BYTES, 100_000, 8_000_000)
)
VOICE_ALLOWED_MIMES = {
    "audio/ogg",
    "audio/opus",
...
RUBIKA_CONTROL_FILE = _data_path(
    os.environ.get("RUBIKA_CONTROL_FILE", "rubika_control.json").strip()
)
...
RUBIKA_CONFIRM_TTL_SECONDS = int(
    _float_env("RUBIKA_CONFIRM_TTL_SECONDS", DEFAULT_RUBIKA_CONFIRM_TTL, 60, 3600)
)
MAX_RUBIKA_REFS = int(_float_env("MAX_RUBIKA_REFS", DEFAULT_MAX_RUBIKA_REFS, 10, 5000))
MAX_RUBIKA_MESSAGE_REFS = int(_float_env("MAX_RUBIKA_MESSAGE_REFS", DEFAULT_MAX_RUBIKA_MESSAGE_REFS, 10, 20000))
MAX_RUBIKA_PENDING_ACTIONS = int(_float_env("MAX_RUBIKA_PENDING_ACTIONS", DEFAULT_MAX_RUBIKA_PENDING_ACTIONS, 1, 1000))
RUBIKA_CHAT_REF_TTL_SECONDS = int(
    _float_env("RUBIKA_CHAT_REF_TTL_SECONDS", DEFAULT_RUBIKA_CHAT_REF_TTL, 3600, 90 * 86400)
)
RUBIKA_MESSAGE_REF_TTL_SECONDS = int(
    _float_env("RUBIKA_MESSAGE_REF_TTL_SECONDS", DEFAULT_RUBIKA_MESSAGE_REF_TTL, 1800, 30 * 86400)
)
...
#  1. _lock_init         - Module initialization (load_all)
#  2. _lock_api_key      - API key rotation
#  3. _lock_hist         - Chat histories (chat_histories, agent_chat_histories)
#  4. _lock_kb           - Knowledge base
#  5. _lock_pending      - Pending replies
#  6. _lock_logs         - Chat logs
#  7. _lock_sent         - Bot sent message IDs
#  8. _automation_lock   - Server automation (reminders, monitors, outbox)
#  9. _rubika_control_lock - Rubika control actions
#  10. _agent_memory_lock  - Agent memory file
#  11. _agent_audit_lock   - Agent audit log
#  12. _grounding_state_lock - Search grounding circuit breaker
#  13. _tts_cache_lock     - TTS cache
#  14. _voice_processing_semaphore - Voice processing concurrency (semaphore, not lock)
...
_voice_processing_semaphore = threading.BoundedSemaphore(DEFAULT_VOICE_PROCESSING_CONCURRENCY)
_tts_cache = OrderedDict()
_tts_cache_timestamps = {}  # Track creation time for TTL
...
BOT_PERSONA = f"""
تو دستیار هوشمند و شخصی {OWNER_NAME} هستی که روی اکانت روبیکای اون فعالیت می‌کنی و دسترسی کامل به موتور جست‌وجوی Google و اینترنت زنده داری (دقیقا مثل خود جمنای واقعی).
با لحن صمیمی، دوستانه و به فارسی جواب بده.
جواب‌ها کوتاه، دقیق و طبیعی باشن.

قوانین:
- برای هرگونه سوال عمومی، اخبار روز، قیمت‌ها (دلار، طلا، سکه، ارز، خودرو، رمزارزها، نفت)، آب‌وهوا، ورزش و نتایج مسابقات، تکنولوژی، وضعیت فعلی و هر سوالی که نیاز به اطلاعات زنده و اینترنت دارد، حتماً از ابزار search_web برای جست‌وجو در گوگل و اینترنت استفاده کن تا دقیق‌ترین اطلاعات روز را با ذکر منبع ارائه دهی.
...
- اتمام کار را با complete_task و حذف را فقط با درخواست صریح مالک با delete_task انجام بده.
- برای خلاصهٔ روز از daily_briefing و برای فرستادن خودکار هر روز از schedule_daily_briefing (و لغو با cancel_daily_briefing) استفاده کن.
- کار و یادآوری فرق دارند: کار = فهرست انجام‌دادنی، یادآوری = پیام زمان‌دار. اگر کاربر «یادم بنداز» گفت یادآوری بساز.
"""
...
def _atomic_write_json(path, data):
    ...
    # Use NamedTemporaryFile for better atomicity and cross-platform safety
...
def _read_json_object(path):
    try:
        if not os.path.exists(path):
            return {}
        if os.path.getsize(path) > MAX_JSON_FILE_SIZE:
...
_KNOWN_NEWS_FEEDS = (
    ...
    (
        ("tasnim", "تسنیم"),
        "تسنیم",
        "https://www.tasnimnews.com/fa/rss/feed/0/8/0/%D9%87%D9%85%D9%87-%D8%AE%D8%A8%D8%B1%D9%87%D8%A7",
    ),
    (
        ("fars", "فارس", "خبرگزاری فارس"),
        "خبرگزاری فارس",
        "https://www.farsnews.ir/rss",
    ),
    (
        ("mehr", "مهر", "خبرگزاری مهر"),
        "خبرگزاری مهر",
        "https://www.mehrnews.com/rss",
    ),
    (
        ("yjc", "یجسی", "-young journalists club"),
        "خبرگزاری یجس",
    ...
    (
        ("khabaronline", "خبرآنلاین", "خبر آنلاین"),
        "خبرآنلاین",
        "https://www.khabaronline.ir/rss",
    ),
    (
        ("aftabnews", "آفتاب", "آفتاب نیوز"),
        "آفتاب نیوز",
        "https://aftabnews.ir/fa/rss/allnews",
    ),
    (
        ("dolat", "دولت", "دولت به زبان ساده"),
        "دولت به زبان ساده",
        "https://dolat.ir/fa/rss/allnews",
    ),
    (
        ("varzesh3", "ورزش سه", "ورزش ۳"),
        "ورزش سه",
        "https://www.varzesh3.com/rss/all",
    ),
    (
        ("khabarvarzeshi", "خبرورزشی"),
        "خبر ورزشی",
        "https://www.khabarvarzeshi.com/rss",
    ),
    (
        ("eghtesadonline", "اقتصاد آنلاین", "اقتصاد آنلاین"),
        "اقتصاد آنلاین",
        "https://eghtesadonline.com/fa/rss/allnews",
    ),
    (
        ("donyaeqtesad", "دنیای اقتصاد", "دنیای اقتصاد"),
        "دنیای اقتصاد",
        "https://donya-eqtesad.com/fa/rss/allnews",
    ),
    (
        ("tejaratnews", "تجارت نیوز", "تجارت نیوز"),
        "تجارت نیوز",
        "https://www.tejaratnews.com/fa/rss/allnews",
    ),
    (
        ("shafa", "شفاعت", "شفاعت نیوز"),
        "شفاعت نیوز",
        "https://shaafa.ir/fa/rss/allnews",
    ),
    (
        ("aryanews", "آریا", "آریا نیوز"),
        "آریا نیوز",
        "https://aryanews.com/fa/rss/allnews",
    ),
)
...
_KNOWN_NEWS_SITEMAPS = (
    ...
    (
        ("irna", "ایرنا"),
        "ایرنا",
        "https://www.irna.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("isna", "ایسنا"),
        "ایسنا",
        "https://www.isna.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("tasnim", "تسنیم"),
        "تسنیم",
        "https://www.tasnimnews.com/sitemap-news.xml",
        "fa",
    ),
    (
        ("fars", "فارس", "خبرگزاری فارس"),
        "خبرگزاری فارس",
        "https://www.farsnews.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("mehr", "مهر", "خبرگزاری مهر"),
        "خبرگزاری مهر",
        "https://www.mehrnews.com/sitemap-news.xml",
        "fa",
    ),
    (
        ("yjc", "یجسی", "young journalists club"),
        "خبرگزاری یجس",
        "https://www.yjc.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("tabnak", "تابناک"),
        "تابناک",
        "https://www.tabnak.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("khabaronline", "خبرآنلاین", "خبر آنلاین"),
        "خبرآنلاین",
        "https://www.khabaronline.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("aftabnews", "آفتاب", "آفتاب نیوز"),
        "آفتاب نیوز",
        "https://aftabnews.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("dolat", "دولت", "دولت به زبان ساده"),
        "دولت به زبان ساده",
        "https://dolat.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("varzesh3", "ورزش سه", "ورزش ۳"),
        "ورزش سه",
        "https://www.varzesh3.com/sitemap-news.xml",
        "fa",
    ),
    (
        ("eghtesadonline", "اقتصاد آنلاین", "اقتصاد آنلاین"),
        "اقتصاد آنلاین",
        "https://eghtesadonline.com/sitemap-news.xml",
        "fa",
    ),
    (
        ("donyaeqtesad", "دنیای اقتصاد", "دنیای اقتصاد"),
        "دنیای اقتصاد",
        "https://donya-eqtesad.com/sitemap-news.xml",
        "fa",
    ),
    (
        ("tejaratnews", "تجارت نیوز", "تجارت نیوز"),
        "تجارت نیوز",
        "https://www.tejaratnews.com/sitemap-news.xml",
        "fa",
    ),
    (
        ("shafa", "شفاعت", "شفاعت نیوز"),
        "شفاعت نیوز",
        "https://shaafa.ir/sitemap-news.xml",
        "fa",
    ),
    (
        ("aryanews", "آریا", "آریا نیوز"),
        "آریا نیوز",
        "https://aryanews.com/sitemap-news.xml",
        "fa",
    ),
)
...
def search_web(query: str) -> str:
    ...
    providers = [
        # Gemini's Grounded Google Search - full internet access like regular Gemini
        ("gemini_google_search", _gemini_google_search),
        # Tavily as backup (if API key available)
        ("tavily", _tavily_search),
        # Official RSS/News sitemaps for known Iranian sites (precise, no API needed)
...
def is_direct_web_request(text):
    """درخواست‌های واضحِ نیازمند اینترنت و گوگل را بدون دور دوم LLM تشخیص می‌دهد."""
    value = " ".join(str(text or "").casefold().split())
    if not value or len(value) < 2:
        return False

    markers = (
        "جستجو", "جست‌وجو", "سرچ", "در وب", "اینترنت", "گوگل کن", "در گوگل", "تو گوگل",
        "قیمت امروز", "قیمت دلار", "قیمت طلا", "قیمت سکه", "قیمت یورو",
        "قیمت بیت کوین", "قیمت ارز", "قیمت بیت‌کوین", "قیمت خودرو", "قیمت نفت",
        "قیمت تتر", "قیمت گوشی", "قیمت درهم", "قیمت پوند",
        "نرخ دلار", "نرخ ارز", "نرخ طلا", "نرخ سکه", "نرخ امروز",
        "دلار چنده", "طلا چنده", "سکه چنده", "بیت کوین چنده", "ارز چنده",
        "آب و هوا", "وضعیت هوا", "هوای ", "هواشناسی",
        "نتیجه بازی", "نتیجه فوتبال", "فوتبال دیشب", "بازی دیشب", "بازی امروز",
        "جدیدترین", "آخرین خبر", "اخبار روز", "خبر فوری", "تازه ترین",
        "شاخص بورس", "بازار بورس", "فرابورس",
        "امروز چندم", "تاریخ امروز", "ساعت چنده", "امروز چندمه", "تعطیل است", "مدارس تعطیل",
    ...
    if "خبر" in value or "اخبار" in value:
        news_qualifiers = (
            ...
            "euronews", "یورونیوز", "یورو نیوز", "euro news", "فوری", "روز",
        )
...
async def _try_handle_direct_web_request(
    ...
    if not is_direct_web_request(user_text):
        return False
    log.info("DIRECT_SEARCH chat=%s text=%r", _mask_guid(chat_guid), user_text[:100])
    try:
        ...
        log.info("DIRECT_SEARCH sent chat=%s owner=%s", _mask_guid(chat_guid), owner_authorized)
    except Exception as exc:
...
def _contains_injection_patterns(text: str) -> bool:
    ...
    cmd_patterns = (
        "; ", "&& ", "|| ", "| ", "`", "$(", "nc ", "netcat",
        "wget ", "curl ", "/bin/", "/etc/passwd",
    )
    # Path traversal
    path_patterns = ("../", "..\\", "/etc/", "c:\\", "c:/")

    for pattern in sql_patterns + cmd_patterns + path_patterns:
...
def _safe_http_fetch(raw_url, method="GET", max_bytes=300_000, timeout=None):
    ...
    for _ in range(4):
        ...
        try:
            with opener.open(request_obj, timeout=timeout, context=_SSL_CONTEXT) as response:
            ...
            if exc.code in {301, 302, 303, 307, 308}:
                ...
                exc.close()
                if not location:
                    raise ValueError("Redirect بدون مقصد دریافت شد.") from exc
...
def _parse_schedule_time(value):
    ...
    if relative:
        ...
        try:
            ...
            # دستیار شخصی: قبل از خطا، زمان‌های طبیعی فارسی را امتحان کن
            # («فردا ساعت ۸ صبح»، «شنبه ظهر»، «۲۲ مرداد ساعت ۹»، «۱۴۰۴/۵/۲۲»).
            natural = parse_persian_datetime(text, now)
            if natural is None:
                raise ValueError(
                    "زمان باید ISO 8601، نسبی (10m/2h/1d) یا فارسی مثل «فردا ساعت ۸ صبح» باشد."
                ) from exc
            target = natural
...
def _server_file_path(filename):
    ...
    root = os.path.realpath(os.path.abspath(SERVER_FILES_DIR))
...
# ════════════════════════════════════════
#  🧠 لایهٔ دستیار شخصی: تقویم شمسی، زمان طبیعی فارسی،
#     کارها (To-Do)، عادت‌ها و بریفینگ روزانه
# ════════════════════════════════════════
...
# نگاشت نام روز هفته به مقدار weekday() پایتون
_PERSIAN_WEEKDAY_TO_INDEX = {
    "شنبه": 5,
    "يكشنبه": 6, "یکشنبه": 6, "یک شنبه": 6,
    "دوشنبه": 0, "دو شنبه": 0,
    "سه‌شنبه": 1, "سه شنبه": 1, "سشنبه": 1,
    "چهارشنبه": 2, "چهار شنبه": 2, "چارشنبه": 2,
    "پنجشنبه": 3, "پنج‌شنبه": 3, "پنج شنبه": 3, "پنجشنبه‌": 3,
    "جمعه": 4, "آدینه": 4,
}
_PERSIAN_DIGIT_TABLE = str.maketrans(
    "۰۱۲۳۴۵۶۷۸۹٠١٢٣٤٥٦٧٨٩", "01234567890123456789"
)
# بخش‌های روز و ساعت پیش‌فرضشان
_DAY_PART_HOURS = (
    ("بامداد", 5),
    ("سحر", 5),
    ("صبح", 8),
...
def _normalize_digits(text) -> str:
    """ارقام فارسی/عربی را به لاتین تبدیل و نیم‌فاصله را یکدست می‌کند."""
    return str(text or "").translate(_PERSIAN_DIGIT_TABLE).replace("\u200c", "\u200c")
...
def _jalali_to_gregorian(jy: int, jm: int, jd: int):
    ...
    gd = days + 1
    month_days = [
        31, 29 if _is_gregorian_leap(gy) else 28, 31, 30, 31, 30,
        31, 31, 30, 31, 30, 31,
    ]
    gm = 1
...
def format_jalali(moment: datetime) -> str:
    """تاریخ شمسی خوانا مثل «سه‌شنبه ۲۲ مرداد ۱۴۰۴»."""
    jy, jm, jd = _gregorian_to_jalali(moment.year, moment.month, moment.day)
    weekday = _PERSIAN_WEEKDAY_NAMES[moment.weekday()]
    return f"{weekday} {jd} {_JALALI_MONTH_NAMES[jm - 1]} {jy}"


def _extract_time_of_day(text: str, default_hour: int, default_minute: int = 0):
    """ساعت را از متن فارسی بیرون می‌کشد: «ساعت ۸»، «۸:۳۰ صبح»، «۹ شب»."""
    hour = None
    minute = 0
    match = re.search(r"(?:ساعت\s*)?(\d{1,2})\s*[:.]\s*(\d{2})", text)
    if match:
        hour = int(match.group(1))
        minute = int(match.group(2))
    else:
        match = re.search(r"ساعت\s*(\d{1,2})", text)
        if not match:
            match = re.search(r"\b(\d{1,2})\s*(?:صبح|ظهر|عصر|شب|بامداد|بعدازظهر)", text)
        if match:
            hour = int(match.group(1))
    part_hour = None
    for name, value in _DAY_PART_HOURS:
        if name in text:
            part_hour = value
            break
    if hour is None:
        return (part_hour if part_hour is not None else default_hour), (
            0 if part_hour is not None else default_minute
        )
    if hour <= 12 and part_hour is not None and part_hour >= 12 and hour != 12:
        # «۹ شب» ⇒ ۲۱
        hour += 12
    if hour > 23:
        hour = 23
...
def parse_persian_datetime(text, now=None):
    """زمان‌های طبیعی فارسی را به datetime تبدیل می‌کند.

    نمونه‌های پشتیبانی‌شده: «فردا ساعت ۸ صبح»، «امشب»، «پس‌فردا ظهر»،
    «شنبه ساعت ۱۰»، «۱۴۰۴/۵/۲۲ ساعت ۹»، «۲۲ مرداد ساعت ۷».
    خروجی None یعنی الگویی شناخته نشد.
    """
    value = " ".join(_normalize_digits(text).split())
    if not value:
        return None
    now = now or datetime.now(SERVER_TZ)
    base = None

    # ۱) تاریخ شمسی عددی: 1404/05/22 یا 1404-5-22
    match = re.search(r"\b(1[34]\d{2})\s*[/\-]\s*(\d{1,2})\s*[/\-]\s*(\d{1,2})\b", value)
    if match:
        jy, jm, jd = (int(match.group(i)) for i in (1, 2, 3))
        if 1 <= jm <= 12 and 1 <= jd <= 31:
            gy, gm, gd = _jalali_to_gregorian(jy, jm, jd)
            base = now.replace(year=gy, month=gm, day=gd)
    # ۲) تاریخ شمسی با نام ماه: «۲۲ مرداد» یا «۲۲ مرداد ۱۴۰۵»
    if base is None:
        for index, name in enumerate(_JALALI_MONTH_NAMES, start=1):
            match = re.search(rf"\b(\d{{1,2}})\s+{name}(?:\s+(1[34]\d{{2}}))?", value)
            if match:
                jd = int(match.group(1))
                jy_now, _, _ = _gregorian_to_jalali(now.year, now.month, now.day)
                jy = int(match.group(2)) if match.group(2) else jy_now
                gy, gm, gd = _jalali_to_gregorian(jy, index, jd)
                base = now.replace(year=gy, month=gm, day=gd)
                if not match.group(2) and base.date() < now.date():
                    gy, gm, gd = _jalali_to_gregorian(jy + 1, index, jd)
                    base = now.replace(year=gy, month=gm, day=gd)
                break
    # ۳) کلیدواژه‌های نسبی
    default_hour, default_minute = 9, 0
    if base is None:
        if "پس فردا" in value or "پس‌فردا" in value or "پسفردا" in value:
        ...
        elif "هفته دیگه" in value or "هفته بعد" in value or "هفتهٔ دیگر" in value:
            base = now + timedelta(days=7)
    # ۴) نام روز هفته ⇒ نزدیک‌ترین رخداد آینده
    if base is None:
        for name, weekday_index in _PERSIAN_WEEKDAY_TO_INDEX.items():
            if name in value:
                ...
                break
    # ۵) فقط بخشی از روز («صبح»، «ظهر»، «شب») بدون تاریخ
    if base is None and any(name in value for name, _ in _DAY_PART_HOURS):
        base = now
    if base is None:
        return None

    hour, minute = _extract_time_of_day(value, default_hour, default_minute)
    target = base.replace(hour=hour, minute=minute, second=0, microsecond=0)
    if target <= now:
        # اگر ساعتِ امروز گذشته، به فردا منتقل می‌شود (رفتار مورد انتظار کاربر)
        if "امروز" not in value and "امشب" not in value:
...
def _clean_task_title(title):
    value = " ".join(str(title or "").split())[:300]
    return value


def add_task(title: str, due: str = "", priority: str = "normal", tags: str = "") -> str:
    """Add a personal to-do task for the owner.

    Args:
        title: Short task description in the user's language.
        due: Optional deadline; ISO 8601, relative (10m/2h/3d) or Persian text like «فردا ساعت ۹».
        priority: low, normal, or high.
        tags: Optional comma-separated tags such as «کار,خرید».
    """
    clean_title = _clean_task_title(title)
    if not clean_title:
        return "عنوان کار خالی است."
    if _contains_secret("task", clean_title):
        return "ذخیرهٔ رمز یا کلید در کارها ممنوع است."
    priority_value = str(priority or "normal").strip().casefold()
    aliases = {
        "low": "low", "کم": "low", "پایین": "low",
        "normal": "normal", "معمولی": "normal", "متوسط": "normal",
        "high": "high", "زیاد": "high", "بالا": "high", "فوری": "high", "مهم": "high",
    }
    priority_value = aliases.get(priority_value, "normal")
    due_ts = 0.0
    due_label = ""
    if str(due or "").strip():
        try:
            due_dt = _parse_schedule_time(due)
            due_ts = due_dt.timestamp()
            due_label = due_dt.strftime("%Y-%m-%d %H:%M")
        except ValueError as exc:
            return f"مهلت نامعتبر است: {exc}"
    tag_list = [t.strip()[:30] for t in str(tags or "").split(",") if t.strip()][:5]
    task_id = uuid.uuid4().hex[:8]
    chat_guid = str(getattr(_agent_context, "chat_guid", ""))[:120]
    with _automation_lock:
        state = _load_automation_locked()
        open_tasks = sum(
            1 for item in state[_task_state_key()].values() if item.get("status") == "open"
        )
        if open_tasks >= MAX_TASKS:
            return "سقف کارهای باز پر شده است؛ اول چند مورد را ببند."
        state[_task_state_key()][task_id] = {
            "id": task_id,
            "title": clean_title,
            "status": "open",
        ...
        _save_automation_locked(state)
    _audit_tool("add_task", "ok", f"id={task_id}")
    suffix = f" (مهلت: {due_label})" if due_label else ""
    return f"✅ کار «{clean_title}» با شناسهٔ {task_id} ثبت شد{suffix}."


def _sorted_open_tasks(state):
    priority_rank = {"high": 0, "normal": 1, "low": 2}
    items = [i for i in state[_task_state_key()].values() if i.get("status") == "open"]
    items.sort(
        key=lambda item: (
            priority_rank.get(item.get("priority", "normal"), 1),
            float(item.get("due") or float("inf")),
...
def list_tasks(scope: str = "open") -> str:
    ...
    Args:
        scope: open, done, today, or overdue.
    """
    mode = str(scope or "open").strip().casefold()
    now = time.time()
    with _automation_lock:
        state = _load_automation_locked()
        if mode in {"done", "انجام‌شده", "انجام شده"}:
            items = [
                i for i in state[_task_state_key()].values() if i.get("status") == "done"
            ]
            items.sort(key=lambda item: float(item.get("done_at", 0)), reverse=True)
        else:
            items = _sorted_open_tasks(state)
    if mode in {"today", "امروز"}:
        end_of_day = (
            datetime.now(SERVER_TZ).replace(hour=23, minute=59, second=59).timestamp()
        )
        items = [i for i in items if i.get("due") and float(i["due"]) <= end_of_day]
    elif mode in {"overdue", "عقب‌افتاده", "عقب افتاده"}:
        items = [i for i in items if i.get("due") and float(i["due"]) < now]
    if not items:
        return "کاری در این دسته وجود ندارد."
    lines = []
    for item in items[:40]:
        due_ts = float(item.get("due") or 0)
        due_text = ""
        if due_ts:
            due_dt = datetime.fromtimestamp(due_ts, SERVER_TZ)
            late = " ⚠️گذشته" if due_ts < now and item.get("status") == "open" else ""
            due_text = f" | مهلت: {format_jalali(due_dt)} {due_dt.strftime('%H:%M')}{late}"
        icon = {"high": "🔴", "normal": "🔵", "low": "⚪"}.get(item.get("priority"), "🔵")
        done = "✔️ " if item.get("status") == "done" else ""
        tags = " ".join(f"#{t}" for t in item.get("tags") or [])
        lines.append(f"{icon} {done}[{item['id']}] {item['title']}{due_text} {tags}".rstrip())
    return "\n".join(lines)


def complete_task(task_id: str) -> str:
    """Mark one task as done by its exact 8-character ID."""
    clean_id = str(task_id or "").strip()
    with _automation_lock:
        state = _load_automation_locked()
        item = state[_task_state_key()].get(clean_id)
        if not item or item.get("status") != "open":
            return "کار بازی با این شناسه پیدا نشد."
        item["status"] = "done"
        item["done_at"] = time.time()
        _save_automation_locked(state)
    _audit_tool("complete_task", "ok", f"id={clean_id}")
    return f"🎉 کار «{item['title']}» انجام‌شده علامت خورد."


def delete_task(task_id: str) -> str:
    """Delete one task permanently by its exact ID."""
    clean_id = str(task_id or "").strip()
    with _automation_lock:
        state = _load_automation_locked()
        item = state[_task_state_key()].pop(clean_id, None)
        if not item:
            return "کاری با این شناسه پیدا نشد."
        _save_automation_locked(state)
    _audit_tool("delete_task", "ok", f"id={clean_id}")
    return f"🗑️ کار «{item.get('title', clean_id)}» حذف شد."


def _format_briefing_news(raw, limit=4):
    """خروجی JSON ابزار جستجو را به چند تیتر خوانا تبدیل می‌کند."""
    try:
    ...
    if not out and answer:
        out.append(f"  • {answer[:200]}")
    return "\n".join(out)


def _briefing_sections(include_web=True):
    """اجزای بریفینگ را جمع می‌کند؛ هر بخش مستقل خطاپذیر است."""
    now = datetime.now(SERVER_TZ)
    lines = [
        f"🗓️ {format_jalali(now)} — ساعت {now.strftime('%H:%M')} ({SERVER_TIMEZONE_NAME})",
    ]
    ...
    try:
        with _automation_lock:
            state = _load_automation_locked()
            reminders = [
                item for item in state["reminders"].values()
                if item.get("active") and float(item.get("next_run", 0)) <= end_of_day
            ]
            tasks = _sorted_open_tasks(state)
            monitors = [i for i in state["monitors"].values() if i.get("active")]
        reminders.sort(key=lambda item: float(item.get("next_run", 0)))
        if reminders:
            lines.append("\n⏰ یادآوری‌های امروز:")
            for item in reminders[:10]:
                when = datetime.fromtimestamp(float(item["next_run"]), SERVER_TZ)
                lines.append(f"  • {when.strftime('%H:%M')} — {item.get('message', '')}")
        else:
            lines.append("\n⏰ یادآوری‌ای برای امروز ثبت نشده.")

        overdue = [t for t in tasks if t.get("due") and float(t["due"]) < now.timestamp()]
        due_today = [
            t for t in tasks
            if t.get("due") and now.timestamp() <= float(t["due"]) <= end_of_day
        ]
        if overdue or due_today or tasks:
            lines.append(f"\n✅ کارها ({len(tasks)} باز):")
            for item in (overdue + due_today or tasks)[:8]:
                mark = "⚠️" if item in overdue else "•"
                lines.append(f"  {mark} [{item['id']}] {item['title']}")
        if monitors:
            unhealthy = [m for m in monitors if m.get("last_error")]
            lines.append(
                f"\n📡 مانیتورها: {len(monitors)} فعال"
                + (f" | {len(unhealthy)} مورد خطا دارد" if unhealthy else " | همه سالم")
            )
    except Exception as exc:  # بریفینگ نباید به‌خاطر یک بخش کامل شکست بخورد
        log.error("BRIEFING STATE ERROR: %s", exc)
        lines.append("\n⚠️ خواندن یادآوری‌ها/کارها ناموفق بود.")

    if include_web:
        try:
            headlines = _format_briefing_news(search_web("مهم‌ترین خبرهای امروز ایران و جهان"))
            if headlines:
                lines.append(f"\n📰 تیترهای امروز:\n{headlines}")
        except Exception as exc:
            log.error("BRIEFING NEWS ERROR: %s", exc)
    return "\n".join(lines)


def daily_briefing(include_news: bool = True) -> str:
    """Build the owner's daily briefing: Persian date, today's reminders, open tasks, monitors, news.

    Args:
        include_news: Set false to skip the web news summary and answer faster.
    """
    result = _briefing_sections(include_web=bool(include_news))
    _audit_tool("daily_briefing", "ok", f"news={bool(include_news)}")
    return result


def schedule_daily_briefing(at: str = "08:00") -> str:
    """Schedule (or reschedule) the recurring daily briefing.

    Args:
        at: Local time of day in HH:MM, for example 07:30.
    """
    value = _normalize_digits(at).strip()
    match = re.fullmatch(r"(\d{1,2})(?::(\d{2}))?", value)
    if not match:
        return "ساعت باید به شکل HH:MM باشد."
    hour = min(23, int(match.group(1)))
    minute = min(59, int(match.group(2) or 0))
    now = datetime.now(SERVER_TZ)
    ...
    chat_guid = str(getattr(_agent_context, "chat_guid", ""))[:120]
    if not _automation_targets(chat_guid):
        return "برای تحویل بریفینگ، چت مبدأ یا OWNER_CONTROL_GROUP لازم است."
    with _automation_lock:
        state = _load_automation_locked()
        for item in state["reminders"].values():
            if item.get("kind") == "briefing" and item.get("active"):
                item["active"] = False
                item["cancelled_at"] = time.time()
        reminder_id = uuid.uuid4().hex[:10]
        state["reminders"][reminder_id] = {
            "id": reminder_id,
            "kind": "briefing",
            "message": "__daily_briefing__",
            "next_run": target.timestamp(),
            "repeat_seconds": SECONDS_PER_DAY,
            "active": True,
            "chat_guid": chat_guid,
            "actor": _agent_actor(),
            "created_at": time.time(),
        ...
        _save_automation_locked(state)
    _audit_tool("schedule_daily_briefing", "ok", f"at={hour:02d}:{minute:02d}")
    return (
        f"📋 بریفینگ روزانه هر روز ساعت {hour:02d}:{minute:02d} فرستاده می‌شود "
        f"(اولین بار: {format_jalali(target)})."
    )


def cancel_daily_briefing() -> str:
    """Cancel the recurring daily briefing."""
    removed = 0
    with _automation_lock:
        state = _load_automation_locked()
        for item in state["reminders"].values():
        ...
        if removed:
            _save_automation_locked(state)
    _audit_tool("cancel_daily_briefing", "ok", f"count={removed}")
    return "بریفینگ روزانه لغو شد." if removed else "بریفینگ روزانهٔ فعالی وجود ندارد."
...
async def synthesize_speech(text):
    ...
    with _tts_cache_lock:
        # باگ #15: جاروی TTL فقط در حالت cache-miss اجرا می‌شد؛ روی یک متن پرتکرار
        # کش هیچ‌وقت پاک نمی‌شد. حالا قبل از هر چیز TTL بررسی می‌شود.
        _clean_tts_cache_locked(now)
        cached = _tts_cache.get(cache_key)
        if cached:
            _tts_cache.move_to_end(cache_key)
            _tts_cache_timestamps[cache_key] = now
            return cached

    chunks = []
    total_bytes = 0
    communicator = edge_tts.Communicate(clean_text, VOICE_TTS_VOICE)
    # باگ #15: قبلاً وقتی حجم خروجی از سقف رد می‌شد، استثنا از داخل `async for`
    # پرتاب می‌شد و استریم/وب‌سوکت edge-tts هیچ‌وقت بسته نمی‌شد (نشت اتصال).
    ...
    with _tts_cache_lock:
        ...
        _tts_cache_timestamps[cache_key] = now
        _tts_cache.move_to_end(cache_key)
        # Enforce limits: count, total bytes, TTL
        _enforce_tts_cache_limits_locked()
    _audit_tool("voice_synthesis", "ok", f"bytes={len(audio)}")
    return audio


def _clean_tts_cache_locked(now: float):
    """Remove expired entries from TTS cache. Must hold _tts_cache_lock."""
    expired = [
        key for key, ts in _tts_cache_timestamps.items()
        if now - ts > TTS_CACHE_TTL_SECONDS
    ]
    for key in expired:
        _tts_cache.pop(key, None)
        _tts_cache_timestamps.pop(key, None)


def _enforce_tts_cache_limits_locked():
    """Enforce cache size and byte limits. Must hold _tts_cache_lock."""
    # Limit by count
    while len(_tts_cache) > TTS_CACHE_MAX_ITEMS:
        old_key, _ = _tts_cache.popitem(last=False)
        _tts_cache_timestamps.pop(old_key, None)
    # Limit by total bytes
    total_bytes = sum(len(v) for v in _tts_cache.values())
    while total_bytes > TTS_CACHE_MAX_BYTES and _tts_cache:
        old_key, old_val = _tts_cache.popitem(last=False)
        _tts_cache_timestamps.pop(old_key, None)
        total_bytes -= len(old_val)
...
def _cleanup_rubika_control_locked(state):
    now = time.time()
    for _code, item in list(state["pending"].items()):
...
def get_current_datetime() -> str:
    """Return the current date and time, both Gregorian ISO and Persian (Jalali)."""
    now = datetime.now(SERVER_TZ)
...
AGENT_TOOLS = GENERAL_TOOLS + [
    # ── ابزارهای دستیار شخصی ──
    add_task,
    list_tasks,
    complete_task,
    delete_task,
...
def switch_api_key():
    ...
    # Copy keys first to avoid holding lock during long operation
    chat_keys = []
    agent_keys = []
    with _lock_hist:
        chat_keys = list(chat_histories.keys())
        agent_keys = list(agent_chat_histories.keys())

    for guid in chat_keys:
        with _lock_hist:
            chat_obj = chat_histories.get(guid)
        if chat_obj and model:
            try:
                new_session = model.start_chat(
                    history=chat_obj.history,
                    enable_automatic_function_calling=True,
                )
                with _lock_hist:
                    if guid in chat_histories:
                        chat_histories[guid] = new_session
            except Exception as exc:
                log.error("خطا در بروزرسانی سشن چت %s: %s", guid, exc)

    for guid in agent_keys:
        with _lock_hist:
            chat_obj = agent_chat_histories.get(guid)
        if chat_obj and agent_model:
            try:
                ...
                with _lock_hist:
                    if guid in agent_chat_histories:
                        agent_chat_histories[guid] = new_session
            except Exception as exc:
...
def _is_rate_limit_error(exc) -> bool:
    """باگ #12: تنها مرجع تشخیص خطای سهمیه/محدودیت Gemini.

    قبلاً سه پیاده‌سازی جدا وجود داشت؛ دوتای‌شان مارکر «rate» را داشتند که با
    پیام‌هایی مثل «moderate» یا «accurate» هم مچ می‌شد و باعث چرخش بی‌دلیل کلید
    (و سوختن کلیدهای سالم) می‌شد. حالا همه از همین تابع استفاده می‌کنند.
    """
    try:
...
# باگ #14: MAX_BOT_SENT_IDS اینجا دوباره تعریف شده بود و مقدار بالایی را بی‌صدا
# بازنویسی می‌کرد؛ حذف شد. این دو هم از DEFAULT_*های بالا می‌آیند.
MAX_CHAT_HISTORIES = int(
    _float_env("MAX_CHAT_HISTORIES", DEFAULT_MAX_CHAT_HISTORIES, 5, 2000)
)
MAX_TURNS = int(_float_env("MAX_TURNS", DEFAULT_MAX_TURNS, 2, 100))
CHAT_HISTORY_TTL_SECONDS = int(_float_env("CHAT_HISTORY_TTL_SECONDS", 6 * 3600, 600, 7 * 86400))
...
# آخرین زمان استفاده از هر سشن، برای پاک‌سازی TTL محور
_chat_history_seen: dict[str, float] = {}
# Use OrderedDict for LRU tracking of bot sent message IDs
...
def _extract_msg_id(obj) -> str | None:
    ...
    for attr in ("message_id", "id"):
        mid = norm_id(getattr(obj, attr, None))
        if mid is not None:
            return mid
    msg_up = getattr(obj, "message_update", None)
    if msg_up is not None:
        mid = norm_id(getattr(msg_up, "message_id", None))
        if mid is not None:
            return mid
    if isinstance(obj, dict):
        mid = norm_id(
            obj.get("message_id")
            or obj.get("id")
            or obj.get("data", {}).get("message_id")
            or obj.get("data", {}).get("message_update", {}).get("message_id")
            or obj.get("message_update", {}).get("message_id")
        )
        if mid is not None:
            return mid
    return None
...
def get_reply_to_id(update) -> str | None:
    ...
    if val is None:
        try:
            ...
            val = msg.get("reply_to_message_id") or msg.get("reply_message_id")
        except Exception:
            pass
    return norm_id(val)
...
def load_json(path, default):
    if os.path.exists(path):
        try:
            if os.path.getsize(path) > MAX_JSON_FILE_SIZE:
                log.error("LOAD ERROR %s: فایل بیش از حد بزرگ است", path)
                return default
        except OSError:
            pass
        try:
...
def save_json(path, data):
    """باگ #13: قبلاً دو پیاده‌سازی نوشتن اتمیک وجود داشت (این یکی فایل موقت را در
    صورت خطا باقی می‌گذاشت). حالا هر دو مسیر از _atomic_write_json استفاده می‌کنند."""
    try:
    ...
    with _lock_pending:
        ...
        for k, v in raw.items():
            if isinstance(v, dict):
                v_copy = dict(v)
                v_copy["message_id"] = norm_id(v_copy.get("message_id"))
                pending_replies[str(k)] = v_copy

    with _lock_sent:
        raw_ids = load_json(BOT_SENT_FILE, [])
        # Use OrderedDict for LRU tracking - preserve insertion order
        bot_sent_message_ids = OrderedDict((str(x), None) for x in raw_ids)
...
def save_bot_sent():
    with _lock_sent:
        snapshot = list(bot_sent_message_ids)
    save_json(BOT_SENT_FILE, snapshot)
...
def _trim_bot_sent_ids():
    ...
    Uses OrderedDict popitem(last=False) to remove OLDEST entries (LRU)."""
    while len(bot_sent_message_ids) > MAX_BOT_SENT_IDS:
        bot_sent_message_ids.popitem(last=False)  # Remove oldest (LRU)


_bot_sent_dirty = False
_bot_sent_last_flush = 0.0
BOT_SENT_FLUSH_SECONDS = 10.0


def flush_bot_sent(force=False):
    """نوشتن دیسک را debounce می‌کند تا هر پیام ارسالی یک بازنویسی کامل فایل نسازد."""
    global _bot_sent_dirty, _bot_sent_last_flush
    with _lock_sent:
        if not _bot_sent_dirty:
            return
        now = time.monotonic()
        if not force and now - _bot_sent_last_flush < BOT_SENT_FLUSH_SECONDS:
            return
        _bot_sent_dirty = False
        _bot_sent_last_flush = now
        snapshot = list(bot_sent_message_ids)
    save_json(BOT_SENT_FILE, snapshot)


def _add_bot_sent_id(msg_id):
    """Add a bot sent message ID with LRU tracking.

    باگ #7: قبلاً به‌ازای هر پیام کل فایل JSON (تا ۵۰۰۰ آیتم) بازنویسی می‌شد و
    شناسه‌ها گاهی int و گاهی str ذخیره می‌شدند؛ پس تشخیص «ریپلای به ربات» خطا می‌داد.
    """
    key = norm_id(msg_id)
    if not key:
        return
    global _bot_sent_dirty
    with _lock_sent:
        bot_sent_message_ids[key] = None
        bot_sent_message_ids.move_to_end(key)
        _trim_bot_sent_ids()
        _bot_sent_dirty = True
    flush_bot_sent()
...
SESSION_NAME = _data_path("my_rubika_account")
SESSION_FILE = SESSION_NAME + ".rp"
    ...
    with _automation_lock:
        ...
        for reminder in state["reminders"].values():
            if not reminder.get("active") or float(reminder.get("next_run", 0)) > now:
                continue
            if reminder.get("kind") == "briefing":
                # متن بریفینگ خارج از لاک ساخته می‌شود (جستجوی وب کند است)؛
                # اینجا فقط زمان‌بندی جلو می‌رود.
                briefing_due = True
                briefing_chat = reminder.get("chat_guid", "")
                briefing_id = reminder.get("id", "")
            else:
        ...
        _save_automation_locked(state)
    if briefing_due:
        # ساخت بریفینگ (شامل خبر) بیرون از لاک تا حلقهٔ اتوماسیون بلاک نشود.
        try:
            text = _briefing_sections(include_web=True)
        except Exception as exc:
            log.error("BRIEFING BUILD ERROR: %s", exc)
            text = _briefing_sections(include_web=False)
        with _automation_lock:
            state = _load_automation_locked()
            _queue_outbox_locked(
                state, f"📋 بریفینگ روزانه\n\n{text}", briefing_chat, "briefing", briefing_id
            )
            _save_automation_locked(state)
    return created
...
def _deliver_one_outbox():
    ...
    if ok and send_result is not None:
        # باگ #4: به‌جای تکرار منطق، از همان helper مشترک استفاده می‌کنیم.
        _add_bot_sent_id(_extract_msg_id(send_result))
...
def _cleanup_expired_automation():
    """Clean up expired reminders, monitors, and completed outbox events."""
    now = time.time()
    with _automation_lock:
        state = _load_automation_locked()
        # Remove expired reminders (completed or cancelled > 1 day ago)
        for rid, reminder in list(state["reminders"].items()):
            if not reminder.get("active") and reminder.get("completed_at", 0):
                if now - reminder["completed_at"] > 86400:  # 1 day
                    state["reminders"].pop(rid, None)
            elif reminder.get("cancelled_at", 0):
                if now - reminder["cancelled_at"] > 86400:
                    state["reminders"].pop(rid, None)

        # Remove cancelled monitors > 1 day ago
        for mid, monitor in list(state["monitors"].items()):
            if not monitor.get("active") and monitor.get("cancelled_at", 0):
                if now - monitor["cancelled_at"] > 86400:
                    state["monitors"].pop(mid, None)

        # Remove completed outbox events > 1 hour ago
        for eid, event in list(state["outbox"].items()):
            if event.get("completed_at", 0):
                if now - event["completed_at"] > 3600:  # 1 hour
                    state["outbox"].pop(eid, None)

        _save_automation_locked(state)


def _cleanup_stale_chat_histories():
    """Remove chat histories that haven't been touched for a long time.

    باگ #5: این تابع یک stub خالی بود ولی هر ~۵۰۰ ثانیه صدا زده می‌شد؛ نتیجه اینکه
    سشن‌های Gemini تا سقف MAX_CHAT_HISTORIES برای همیشه در حافظه می‌ماندند.
    """
    cutoff = time.time() - CHAT_HISTORY_TTL_SECONDS
    removed = 0
    with _lock_hist:
        for store in (chat_histories, agent_chat_histories):
            for key in [k for k, ts in _chat_history_seen.items() if ts < cutoff]:
                if store.pop(key, None) is not None:
                    removed += 1
        live = set(chat_histories) | set(agent_chat_histories)
        for key in [k for k in _chat_history_seen if k not in live]:
            _chat_history_seen.pop(key, None)
    # پاک‌سازی سطل‌های خالی rate limiter تا حافظه بی‌نهایت رشد نکند.
    now_mono = time.monotonic()
    with _rate_limit_lock:
        for key in [
            k for k, hits in _rate_limit_store.items()
            if not hits or now_mono - hits[-1] > 300
        ]:
            _rate_limit_store.pop(key, None)
    if removed:
        log.info("CLEANUP  %s سشن چت راکد آزاد شد", removed)
    # فلاش نهایی شناسه‌های ارسالی که هنوز روی دیسک نرفته‌اند.
    flush_bot_sent(force=True)
...
def _automation_loop():
    ...
    while True:
        try:
            ...
            # Periodic cleanup every 100 iterations (~500 seconds with 5s interval)
            cleanup_counter += 1
            if cleanup_counter >= 100:
                cleanup_counter = 0
                _cleanup_expired_automation()
                _cleanup_stale_chat_histories()
        except Exception as exc:
...
def _parse_assistant_command(original, value):
    """دستورهای فارسی دستیار شخصی (کارها + بریفینگ) بدون مصرف سهمیهٔ Gemini."""
    normalized = _normalize_digits(original)
    low = normalized.casefold()

    # بریفینگ
    if any(m in low for m in ("بریفینگ", "خلاصه روزم", "خلاصهٔ روزم", "گزارش روز")):
        if any(m in low for m in ("لغو", "خاموش", "غیرفعال", "قطع")):
            return "cancel_briefing", {}
        match = re.search(r"(?:ساعت|هر روز)\s*(\d{1,2})(?::(\d{2}))?", normalized)
        if match and any(m in low for m in ("هر روز", "روزانه", "تنظیم", "زمان‌بندی", "زمانبندی")):
            hour = match.group(1)
            minute = match.group(2) or "00"
            return "schedule_briefing", {"at": f"{hour}:{minute}"}
        return "briefing", {"news": "خبر" not in low or "بی‌خبر" not in low}

    # اتمام کار: «کار a1b2c3d4 انجام شد» / «کار انجام شد a1b2c3d4»
    match = re.search(
        r"(?:کار|تسک)\s*([0-9a-f]{8})\s*(?:رو\s*)?(?:انجام شد|تمام شد|تموم شد|بستم|ok)",
        low,
    ) or re.search(r"(?:انجام شد|تمام شد|بستم)\s*(?:کار|تسک)?\s*([0-9a-f]{8})", low)
    if match:
        return "complete_task", {"id": match.group(1)}

    # حذف کار
    match = re.search(r"(?:حذف|پاک کن|پاک)\s*(?:کار|تسک)\s*([0-9a-f]{8})", low)
    if match:
        return "delete_task", {"id": match.group(1)}

    # فهرست کارها
    if any(m in low for m in ("لیست کارها", "کارهام", "کارهایم", "لیست تسک", "تسک‌هام", "کارهای من")):
        if "انجام" in low and "شده" in low:
            return "list_tasks", {"scope": "done"}
        if "امروز" in low:
            return "list_tasks", {"scope": "today"}
        if "عقب" in low or "گذشته" in low:
            return "list_tasks", {"scope": "overdue"}
        return "list_tasks", {"scope": "open"}

    # افزودن کار: «اضافه کن به کارهام ...» / «کار جدید: ...» / «تسک: ...»
    match = re.search(
        r"(?:کار جدید|تسک جدید|اضافه کن به کارهام|به کارهام اضافه کن|یه کار اضافه کن)\s*[:：]?\s*(.+)",
        normalized,
    )
    if match:
        body = match.group(1).strip(" :،")
        if body:
            priority = "high" if any(m in low for m in ("فوری", "مهم", "اولویت بالا")) else "normal"
            due = ""
            parsed = parse_persian_datetime(body)
            if parsed is not None:
                due = parsed.isoformat(timespec="minutes")
            return "add_task", {"title": body[:300], "due": due, "priority": priority}
    return None
...
def parse_server_command(text):
    ...
    assistant = _parse_assistant_command(original, value)
    if assistant:
        return assistant
...
def execute_direct_server_command(command, actor, chat_guid=""):
    ...
    try:
        ...
        # ── دستیار شخصی ──
        if action == "add_task":
            return add_task(args["title"], args.get("due", ""), args.get("priority", "normal"))
        if action == "list_tasks":
            return list_tasks(args.get("scope", "open"))
        if action == "complete_task":
            return complete_task(args["id"])
        if action == "delete_task":
            return delete_task(args["id"])
        if action == "briefing":
            return daily_briefing(bool(args.get("news", True)))
        if action == "schedule_briefing":
            return schedule_daily_briefing(args.get("at", "08:00"))
        if action == "cancel_briefing":
            return cancel_daily_briefing()
...
def payload_too_large(e):
    return jsonify({"error": "Payload too large", "message": "Request body exceeds limit"}), 413
...
def protect_dashboard_and_api():
    ...
    # health برای health-check سرویس میزبانی عمومی، بدون احراز هویت باقی می‌ماند.
    if request.path == "/api/health":
        return None

    # لینک‌های دانلود با HMAC امضا شده‌اند؛ احراز هویت داشبورد لازم نیست
    # اما باید در برابر brute-force امضا محدود شوند (باگ #2).
    if request.path.startswith("/download/"):
        if not _check_rate_limit(f"download:{client_ip}", DEFAULT_DOWNLOAD_RATE_LIMIT, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
        return None

    # Rate limiting for public endpoints
    if request.path.startswith("/api/"):
        if not _check_rate_limit(f"api:{client_ip}", DEFAULT_API_RATE_LIMIT, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
    else:
        if not _check_rate_limit(f"dash:{client_ip}", DEFAULT_DASHBOARD_RATE_LIMIT, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
...
<div class="app">
  ...
  <main class="main">
    ...
    <!-- ───── Tasks & Briefing Panel ───── -->
    <div id="panel-tasks" class="panel">
      <div class="card">
        <div class="card-header">
          <div class="card-title"><i class="fas fa-list-check"></i> کارهای من</div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">
          <input type="text" id="task-title" class="form-input" placeholder="عنوان کار…" style="flex:2;min-width:180px">
          <input type="text" id="task-due" class="form-input" placeholder="مهلت: فردا ساعت ۹ صبح / 2h" style="flex:1;min-width:150px">
          <select id="task-priority" class="form-input" style="max-width:130px">
            <option value="normal">معمولی</option>
            <option value="high">فوری</option>
            <option value="low">کم</option>
          </select>
          <button class="btn btn-primary" onclick="createTask()"><i class="fas fa-plus"></i> افزودن</button>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">
          <button class="btn" onclick="loadTasks('open')">باز</button>
          <button class="btn" onclick="loadTasks('today')">امروز</button>
          <button class="btn" onclick="loadTasks('overdue')">عقب‌افتاده</button>
          <button class="btn" onclick="loadTasks('done')">انجام‌شده</button>
        </div>
        <div id="tasks-list"></div>
      </div>
      <div class="card" style="margin-top:16px">
        <div class="card-header">
          <div class="card-title"><i class="fas fa-newspaper"></i> بریفینگ روزانه</div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px">
          <button class="btn" onclick="loadBriefing(false)">ساخت بریفینگ</button>
          <button class="btn" onclick="loadBriefing(true)">با خبرهای روز</button>
        </div>
        <pre id="briefing-box" style="white-space:pre-wrap;line-height:1.9;margin:0"></pre>
      </div>
    </div>
...
// ───── Tasks & Briefing ─────
let taskScope='open';
async function loadTasks(scope){
  taskScope=scope||taskScope;
  const list=document.getElementById('tasks-list');
  try{
    const r=await fetch('/api/tasks?scope='+encodeURIComponent(taskScope));
    const d=await r.json(),items=d.tasks||[];
    list.innerHTML='';
    if(!items.length){list.innerHTML='<div class="empty-state"><i class="fas fa-check-circle"></i><p>کاری در این دسته نیست</p></div>';return;}
    const flag={high:'🔴',normal:'🔵',low:'⚪'};
    for(const t of items){
      const div=document.createElement('div');div.className='item-card';
      const due=t.due_jalali?('مهلت: '+t.due_jalali+' — '+(t.due||'').slice(11,16)):'بدون مهلت';
      div.innerHTML='<div class="item-content"><b>'+(flag[t.priority]||'🔵')+' '+esc(t.title)
        +'</b><p>'+esc(due)+' • کد: '+esc(t.id)+'</p></div>';
      if(!t.done){
        const ok=document.createElement('button');ok.className='btn btn-sm';
        ok.innerHTML='<i class="fas fa-check"></i>';ok.title='انجام شد';
        ok.onclick=()=>taskAction('/api/tasks/'+encodeURIComponent(t.id)+'/done','POST');
        div.appendChild(ok);
      }
      const del=document.createElement('button');del.className='btn btn-danger btn-sm';
      del.innerHTML='<i class="fas fa-trash"></i>';del.title='حذف';
      del.onclick=()=>{if(confirm('این کار حذف شود؟'))taskAction('/api/tasks/'+encodeURIComponent(t.id),'DELETE');};
      div.appendChild(del);
      list.appendChild(div);
    }
  }catch(e){list.innerHTML='<div class="empty-state"><p>خطا در دریافت کارها</p></div>';}
}
async function taskAction(url,method){
  try{
    const r=await fetch(url,{method:method});const d=await r.json();
    showToast(d.message||'انجام شد',!d.ok);loadTasks();
  }catch(e){showToast('خطا در ارتباط با سرور',true);}
}
async function createTask(){
  const title=document.getElementById('task-title').value.trim();
  if(!title){showToast('عنوان کار را بنویس',true);return;}
  const body={title:title,due:document.getElementById('task-due').value.trim(),
              priority:document.getElementById('task-priority').value};
  try{
    const r=await fetch('/api/tasks',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
    const d=await r.json();
    showToast(d.message||d.error||'ثبت شد',!d.ok);
    if(d.ok){document.getElementById('task-title').value='';document.getElementById('task-due').value='';}
    loadTasks();
  }catch(e){showToast('خطا در ثبت کار',true);}
}
async function loadBriefing(withNews){
  const box=document.getElementById('briefing-box');
  box.textContent='در حال ساخت بریفینگ…';
  try{
    const r=await fetch('/api/briefing?news='+(withNews?'1':'0'));
    const d=await r.json();
    box.textContent=d.briefing||d.error||'—';
  }catch(e){box.textContent='خطا در ساخت بریفینگ';}
}
...
def api_health_detailed():
    ...
    باگ #9: قبلاً نبودِ هر مؤلفهٔ «اختیاری» (Groq، edge-tts، OWNER_GUIDS، فایل سشن)
    باعث خطای 503 می‌شد و health-check میزبان سرویس را ری‌استارت می‌کرد.
    حالا فقط مؤلفه‌های حیاتی وضعیت 503 می‌سازند.
    """
    critical = {
        "gemini_configured": bool(GEMINI_API_KEYS),
        "dashboard_password_set": bool(DASHBOARD_PASSWORD),
        "rubika_client_ready": main_loop is not None and not main_loop.is_closed(),
    }
    optional = {
        "groq_configured": bool(GROQ_API_KEY),
        "edge_tts_available": edge_tts is not None,
        "rubika_session_exists": os.path.exists(SESSION_FILE),
        "owner_guids_set": bool(OWNER_GUIDS),
        "tavily_configured": bool(TAVILY_API_KEY),
        "public_base_url_set": bool(PUBLIC_BASE_URL),
    }
    healthy = all(critical.values())
    degraded = healthy and not all(optional.values())
    return jsonify({
...
def automation_status():
    with _automation_lock:
        ...
        for item in state["reminders"].values():
            if item.get("kind") == "briefing" and item.get("active"):
                briefing_at = datetime.fromtimestamp(
                    float(item.get("next_run", 0)), SERVER_TZ
                ).strftime("%H:%M")
                break
    now = datetime.now(SERVER_TZ)
    return jsonify({
...
@app.route("/api/tasks")
def api_tasks():
    """فهرست کارهای دستیار شخصی برای داشبورد (فقط خواندنی)."""
    if not _check_rate_limit(f"tasks:{_get_client_ip()}", DEFAULT_API_RATE_LIMIT, SECONDS_PER_MINUTE):
        return jsonify({"error": "تعداد درخواست‌ها زیاد است."}), 429
    scope = (request.args.get("scope") or "open").strip().lower()
    if scope not in {"open", "done", "today", "overdue"}:
        scope = "open"
    now_ts = time.time()
    end_of_day = datetime.now(SERVER_TZ).replace(
        hour=23, minute=59, second=59, microsecond=0
    ).timestamp()
    with _automation_lock:
        state = _load_automation_locked()
        items = list(state[_task_state_key()].values())
    out = []
    for item in items:
        done = item.get("status") == "done"
        due = float(item.get("due") or 0)
        if scope == "done" and not done:
            continue
        if scope != "done" and done:
            continue
        if scope == "today" and not (due and due <= end_of_day):
            continue
        if scope == "overdue" and not (due and due < now_ts):
            continue
        out.append({
            "id": item.get("id", ""),
            ...
            "done": done,
            "due": (
                datetime.fromtimestamp(due, SERVER_TZ).isoformat(timespec="minutes")
                if due else ""
            ),
            "due_jalali": format_jalali(datetime.fromtimestamp(due, SERVER_TZ)) if due else "",
        })
    out.sort(key=lambda i: (i["due"] or "9999", i["title"]))
    return jsonify({"scope": scope, "count": len(out), "tasks": out})


@app.route("/api/tasks", methods=["POST"])
def api_tasks_create():
    """ثبت کار جدید از داشبورد."""
    if not _check_rate_limit(
        f"tasks_w:{_get_client_ip()}", DEFAULT_KB_WRITE_RATE_LIMIT, SECONDS_PER_MINUTE
    ):
        return jsonify({"error": "تعداد درخواست‌ها زیاد است."}), 429
    payload = request.get_json(silent=True) or {}
    title = str(payload.get("title") or "").strip()
    if not title:
        return jsonify({"error": "عنوان کار لازم است."}), 400
    if _contains_injection_patterns(title):
        return jsonify({"error": "ورودی نامعتبر است."}), 400
    _agent_context.actor = "dashboard"
    _agent_context.chat_guid = OWNER_CONTROL_GROUP
    result = add_task(
        title,
        str(payload.get("due") or "").strip(),
        str(payload.get("priority") or "normal"),
        str(payload.get("tags") or ""),
    )
    return jsonify({"ok": not result.startswith(("عنوان", "مهلت", "سقف", "ذخیره")), "message": result})


@app.route("/api/tasks/<task_id>/done", methods=["POST"])
def api_tasks_done(task_id):
    """کار را انجام‌شده علامت می‌زند."""
    if not _check_rate_limit(
        f"tasks_w:{_get_client_ip()}", DEFAULT_KB_WRITE_RATE_LIMIT, SECONDS_PER_MINUTE
    ):
        return jsonify({"error": "تعداد درخواست‌ها زیاد است."}), 429
...
def api_tasks_delete(task_id):
    ...
    ok = result.startswith("🗑️")
    return jsonify({"ok": ok, "message": result}), (200 if ok else 404)


@app.route("/api/briefing")
def api_briefing():
    """بریفینگ روزانه را زنده می‌سازد (پیش‌فرض بدون خبر تا سریع باشد)."""
    if not _check_rate_limit(
        f"briefing:{_get_client_ip()}", DEFAULT_BRIEFING_RATE_LIMIT, SECONDS_PER_MINUTE
    ):
        return jsonify({"error": "تعداد درخواست‌ها زیاد است."}), 429
...
def api_voice_chat():
    # Rate limit: 5 requests/minute per IP for voice (expensive operation)
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"voice:{client_ip}", DEFAULT_VOICE_RATE_LIMIT, 60):
        return jsonify({"error": "Voice rate limit exceeded"}), 429
...
def api_send():
    ...
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"send:{client_ip}", DEFAULT_SEND_RATE_LIMIT, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    ...
    if not guid or not text:
        return jsonify({"error": "Missing guid or text"}), 400
    if len(text) > 4000:
        return jsonify({"error": "Message too long"}), 413
    if _contains_injection_patterns(text) or _contains_injection_patterns(guid):
        log.warning("Potential injection in send from %s", client_ip)
        return jsonify({"error": "Invalid input"}), 400
...
def api_kb():
    ...
    if request.method == "GET":
        if not _check_rate_limit(f"kb_get:{client_ip}", DEFAULT_KB_RATE_LIMIT, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
        with _lock_kb:
            return jsonify({"kb": dict(knowledge_base)})
    else:
        if not _check_rate_limit(f"kb_post:{client_ip}", DEFAULT_KB_WRITE_RATE_LIMIT, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
        data = request.get_json() or {}
...
def api_kb_delete(q):
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"kb_del:{client_ip}", DEFAULT_KB_WRITE_RATE_LIMIT, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    if _contains_injection_patterns(q):
        log.warning("Potential injection in KB delete from %s", client_ip)
        return jsonify({"error": "Invalid input"}), 400
    with _lock_kb:
...
def api_pending():
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"pending:{client_ip}", DEFAULT_PENDING_RATE_LIMIT, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    with _lock_pending:
        return jsonify({
            "pending": {
...
def api_answer():
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"answer:{client_ip}", DEFAULT_ANSWER_RATE_LIMIT, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    if not model:
    ...
    if len(text) > 5000:
        return jsonify({"error": "Input too long"}), 413
    if _contains_injection_patterns(text):
        log.warning("Potential injection in answer from %s", client_ip)
        return jsonify({"error": "Invalid input"}), 400
    ...
    ok, send_result = send_msg_sync(
        original["chat_guid"],
        final_answer,
        reply_to=norm_id(original.get("message_id")),
    )

    # ثبت message_id پیام ارسال‌شده تا بعداً ریپلای‌ها شناسایی بشن
    if ok and send_result is not None:
        try:
            sent_mid = _extract_msg_id(send_result)
            if sent_mid is not None:
                _add_bot_sent_id(str(sent_mid))
    ...
    if ok:
        ...
        # ✅ باگ #8: برگرداندن pending در صورت خطا با کلید رشته‌ای
        with _lock_pending:
...
@app.route("/api/logs")
def api_logs():
    # باگ #6: این مسیر برخلاف بقیهٔ API‌ها هیچ محدودیت نرخی نداشت.
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"logs:{client_ip}", DEFAULT_LOGS_RATE_LIMIT, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    with _lock_logs:
...
def api_config():
    ...
    # باگ #8: خواندن کش TTS باید زیر قفل انجام شود؛ در غیر این‌صورت هم‌زمان با
    # synthesize_speech ممکن است RuntimeError: dictionary changed size بدهد.
    with _tts_cache_lock:
        tts_cache_items = len(_tts_cache)
        tts_cache_bytes = sum(len(item) for item in _tts_cache.values())
    return jsonify({
        "env": {
        ...
        "voice": {
            ...
            "tts_cache_bytes": tts_cache_bytes,
        },
    })
...
def get_agent_chat_session(session_key):
    with _lock_hist:
        _chat_history_seen[session_key] = time.time()
...
def get_chat_session(chat_guid):
    ...
    with _lock_hist:
        _chat_history_seen[chat_guid] = time.time()
...
async def handle_messages(update: Updates):
    ...
    # ✅ باگ #3: درخواست اینترنتی مستقیم (خبر، جستجو، قیمت، هوا و...) برای همه کاربران اجرا می‌شود.
    if await _try_handle_direct_web_request(
        update,
        user_text,
        chat_guid,
    ...
    try:
        ...
        if "از حسن می‌پرسم" in clean or f"از {OWNER_NAME} می‌پرسم" in clean:
            ...
            waiting = looks_like_waiting(clean)
        ...
        if waiting:
            # ✅ باگ #7: استفاده از uuid4 به جای random - use full UUID for better entropy
            ...
            if OWNER_CONTROL_GROUP:
                try:
                    ...
                    if nid is not None:
                        # ✅ باگ #8: تعویض کلید با لاک - atomic compare-and-swap
                        with _lock_pending:
                            # Use get+del to avoid KeyError if already replaced
                            original = pending_replies.get(pending_id)
                            if original is not None:
                                pending_replies[str(nid)] = original
