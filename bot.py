...
async def handle_messages(update: Updates):
    ...
    if is_private:
        ...
        # ✅ درخواست‌های مستقیم وب (خبر، جستجو، قیمت، هوا) برای همه کاربران در همه چت‌ها پردازش می‌شود
        is_direct = is_direct_web_request(user_text)
        log.info("DIRECT_SEARCH_CHECK text=%r is_direct=%s owner=%s private=%s", user_text[:100], is_direct, owner_authorized, chat_guid.startswith("u0"))
        if is_direct:
            log.info("DIRECT_SEARCH early chat=%s text=%r", _mask_guid(chat_guid), user_text[:100])
            try:
                ...
                log.info("DIRECT_SEARCH early sent chat=%s owner=%s", _mask_guid(chat_guid), owner_authorized)
            except Exception as exc:
                log.error("DIRECT SEARCH early ERROR: %s", exc, exc_info=True)
                try:
    ...
    # درخواست اینترنتی مستقیم (خبر، جستجو، قیمت، هوا و...) برای همه کاربران اجرا می‌شود.
    is_direct = is_direct_web_request(user_text)
    log.info("DIRECT_SEARCH_CHECK text=%r is_direct=%s", user_text[:100], is_direct)
    if is_direct:
        try:
            direct_reply = await asyncio.to_thread(
                execute_direct_web_search,
