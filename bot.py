...
DEFAULT_VOICE_RATE_LIMIT = 5
DEFAULT_SEND_RATE_LIMIT = 10
DEFAULT_KB_RATE_LIMIT = 30
...
def execute_with_rotation(func, *args, **kwargs):
    ...
    for attempt in range(max_tries):
        try:
            ...
            err_str = str(e).lower()
            # Check for specific rate limit errors from google-generativeai
            is_rate_limit = any(
                marker in err_str
                for marker in ("429", "quota", "exhausted", "rate", "resource_exhausted")
            )
            # Check for google.api_core.exceptions if available
            try:
...
def _add_bot_sent_id(msg_id: str):
    """Add a bot sent message ID with LRU tracking."""
    with _lock_sent:
        bot_sent_message_ids[msg_id] = None
        bot_sent_message_ids.move_to_end(msg_id)
        _trim_bot_sent_ids()
    save_bot_sent()
...
def _deliver_one_outbox():
    ...
    if ok and send_result is not None:
        ...
        if sent_id:
            with _lock_sent:
                # OrderedDict: use key with None value for LRU tracking
                bot_sent_message_ids[sent_id] = None
                # Move to end to mark as recently used
                bot_sent_message_ids.move_to_end(sent_id)
                _trim_bot_sent_ids()
            save_bot_sent()
...
def _cleanup_stale_chat_histories():
    """Remove chat histories that haven't been used recently."""
    # This is a no-op for now since chat_histories are LRU and bounded by MAX_CHAT_HISTORIES
    # But we could add TTL-based cleanup here if needed
    pass
...
def protect_dashboard_and_api():
    ...
    if request.path == "/api/health":
        pass  # No rate limit on health
    elif request.path.startswith("/api/"):
        # API endpoints: 60 requests/minute per IP
        if not _check_rate_limit(f"api:{client_ip}", 60, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
    else:
        # Dashboard: 30 requests/minute per IP
        if not _check_rate_limit(f"dash:{client_ip}", 30, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
...
def api_chat():
    ...
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"chat:{client_ip}", 20, 60):
...
def api_voice_chat():
    ...
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"voice:{client_ip}", 5, 60):
...
def api_send():
    ...
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"send:{client_ip}", 10, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
...
def api_kb():
    ...
    if request.method == "GET":
        if not _check_rate_limit(f"kb_get:{client_ip}", 30, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
        with _lock_kb:
            return jsonify({"kb": dict(knowledge_base)})
    else:
        if not _check_rate_limit(f"kb_post:{client_ip}", 10, 60):
            return jsonify({"error": "Rate limit exceeded"}), 429
        data = request.get_json() or {}
...
def api_kb_delete(q):
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"kb_del:{client_ip}", 10, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
...
def api_pending():
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"pending:{client_ip}", 30, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    with _lock_pending:
        return jsonify({
            "pending": {
...
def api_answer():
    client_ip = _get_client_ip()
    if not _check_rate_limit(f"answer:{client_ip}", 20, 60):
        return jsonify({"error": "Rate limit exceeded"}), 429
    if not model:
...
def api_config():
    ...
    return jsonify({
        ...
        "voice": {
            ...
            "tts_cache_bytes": sum(len(v) for v in _tts_cache.values()),
        },
    })
...
async def handle_messages(update: Updates):
    ...
    if is_private:
        ...
        # ✅ باگ #3: درخواست‌های مستقیم وب (خبر، جستجو، قیمت، هوا) برای همه کاربران در همه چت‌ها پردازش می‌شود
        ...
        if not is_reply_to_bot and not _TRIGGER_WORD_PATTERN.search(user_text):
            return
